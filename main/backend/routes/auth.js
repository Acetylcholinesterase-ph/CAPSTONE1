...
// Student registration (from machine)
...
router.post('/change-pin', async (req, res) => {
    const { session_token, current_pin, new_pin } = req.body;

    try {
        // Verify session
        const sessionResult = await db.query(`
            SELECT us.user_id, u.pin_code
            FROM user_sessions us
            JOIN users u ON us.user_id = u.id
            WHERE us.session_token = $1 AND us.expires_at > NOW()
        `, [session_token]);
        ...
        // Verify current PIN
        const isValidPin = await bcrypt.compare(current_pin, user.pin_code);
        if (!isValidPin) {
